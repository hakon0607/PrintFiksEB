...
6b. **Vil dere ha de 30 ferdige modellene i galleriet?** Lag en ny query
   (**New query**), åpne filen `supabase/modeller.sql`, kopier alt inn og trykk
   **Run**. Da legges 30 modeller med bilder, beskrivelser og priser inn i
   galleriet. Alt kan endres etterpå under **Admin → Galleri**, og dere kan
   slette de dere ikke vil ha. Filen er trygg å kjøre flere ganger.

6c. Har dere satt opp databasen tidligere (før økonomisiden kom)? Kjør også
   `supabase/okonomi.sql` på samme måte. Den legger til regnskapet og feltet for
   ekstra kostnad per modell. Lager dere databasen fra bunnen nå, er alt med
   allerede i `schema.sql`.
